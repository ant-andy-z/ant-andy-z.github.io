...
We consider a Brownian Motion generated by the limit of scaled random walk. Consider an infinite coin tossing space $(\Omega, \mathcal{F}, \mathbb{P}),$ where $\Omega=\{ \omega_{1}\omega_{2}\dots \}$. The initial position of a traveller is $M(0) = 0.$ For each coin toss $\omega_{i}$, if it is head, the traveller moves forward by 1 unit, otherwise, he moves backward by 1 unit. We find the Random Walk $M(t)$. 
